...
- `App.secondFactor`: enables the second factor in the UI — the login asks for a code when the server refuses the password alone with `SECOND_FACTOR_REQUIRED`, sends a user refused with `SECOND_FACTOR_ENROLMENT_REQUIRED` to `/second_factor/enrol`, and adds _Two-factor authentication_ to the profile menu. Needs an `openimis-be-core` that has the second factor (`tokenAuth(otp)`, `enrolSecondFactor`, `confirmSecondFactor`, `issueRecoveryCodes`); with it off the login page sends and renders exactly what it did before. Default **false**.
