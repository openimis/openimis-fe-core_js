...
- `App.secondFactor`: enables the second factor in the UI — the login asks for a code when the server refuses the password alone with `SECOND_FACTOR_REQUIRED`, sends a user refused with `SECOND_FACTOR_ENROLMENT_REQUIRED` to `/second_factor/enrol`, adds _Two-factor authentication_ to the profile menu, and adds a _Two-factor authentication_ panel to the user administration page that shows whether the user has an authenticator and, for a caller holding right `121705` (`gql_mutation_reset_second_factor_perms`), offers the audited `resetUserSecondFactor` behind a confirmation. Needs an `openimis-be-core` that has the second factor (`tokenAuth(otp)`, `enrolSecondFactor`, `confirmSecondFactor`, `issueRecoveryCodes`, `resetUserSecondFactor`, `users { hasSecondFactor }`); with it off the login page and the user page send and render exactly what they did before. Default **false**.
